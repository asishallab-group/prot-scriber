...
pub struct AnnotationProcess {
    ...
    /// data, i.e. for which `process_query_data_complete` has already run. This is tracked
    /// independently of `human_readable_descriptions` to detect unsorted input (see
    /// `insert_query`): in `AnnotationProcessMode::FamilyAnnotation` `human_readable_descriptions`
    /// is keyed by seq-family-id, not query-id, so it cannot be used for that check in that mode.
...
impl AnnotationProcess {
    ...
    pub fn insert_query(&mut self, qacc: String, query: Query) {
        // panic! if query.id already completed, this means the input SSSR files were not sorted
        // by query identifiers (`qacc` in Blast terminology). Note this must check
        ...
        // query-id, so relying on it would silently miss this exact same problem in family mode.
...
mod tests {
    ...
    fn insert_query_panics_in_case_of_unsorted_blast_table() {
        ...
        // Mark qacc as already completed, simulating that all input SSST files already
        // fully reported this query once before (as happens after a legitimate first
        // completion):
