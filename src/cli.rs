...
pub struct Args {
    ...
    #[arg(
        ...
        long_help = "Regular expressions (regexs) used to recognize non-informative words: such a word is not counted when a query's words are scored, but it is kept in the description, worth a fixed tiny score -- 'prot-scriber doc algorithm' says what that does to a phrase. Each expression is tried against one word at a time, and may match part of it. The value is a file with one expression per line, or '@NAME' for one of prot-scriber's built-in lists -- 'prot-scriber defaults' prints what there is, and '@NAME' is the same list -- or 'none' to hold no word non-informative at all. There is a default list hard-coded into prot-scriber. Write it out to start from it, with 'prot-scriber defaults non-informative-words-regexs > my_non_informative_words_regexs.txt'; nothing needs downloading, and what you get is the list this binary applies. - Note that this is an expert option."
    )]
    pub non_informative_words_regexs: Option<String>,
