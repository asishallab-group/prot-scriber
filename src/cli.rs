...
pub enum Command {
    ...
    #[command(
        subcommand,
        long_about = "Count the words of a whole reference database's annotations, to find the rules a filter list is missing.\n\nAnything a filter list fails to strip is counted as a word, and a word that is really an identifier, a unit or a marker rises straight to the top of the count, where nothing else looks like it. Every rule added to prot-scriber's own lists in August 2026 was found that way.\n\n  prot-scriber corpus build --name sprot --fasta uniprot_sprot.fasta -o sprot.corpus\n  prot-scriber corpus show sprot.corpus\n\nA corpus carries the rules its words were prepared with, so that two of them can be compared and 'corpus diff' can say which rules differ.\n\nNOTHING ELSE READS A CORPUS. An annotation run cannot be given one: what a word is worth is decided among the hits of the one protein being annotated, and only there. Weighting that by a whole database's frequencies was built in August 2026, measured, and removed -- prot-scriber's information content RISES with frequency, which is what makes it a consensus, so a database's commonest words would win. See 'prot-scriber corpus show --help'."
    )]
